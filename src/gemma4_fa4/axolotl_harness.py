...
def _compare_losses(baseline: Mapping[str, Any], candidate: Mapping[str, Any]) -> dict[str, float]:
    left = [float(value) for value in baseline.get("losses", ())]
    right = [float(value) for value in candidate.get("losses", ())]
    if not left or len(left) != len(right):
        raise HarnessComparisonError("loss sequences are empty or have different lengths")
...
def _validate_project_routes(report: Mapping[str, Any]) -> None:
    ...
    for layer_idx in range(48):
        ...
        if set(observed) != {expected_route} or int(observed[expected_route]) <= 0:
            raise HarnessComparisonError(
                f"layer {layer_idx} route is not the required {expected_route}"
            )
...
def compare_reports(
    ...
    _validate_project_routes(candidate)
    loss = _compare_losses(baseline, candidate)
    gradient = _compare_gradient_probe(baseline, candidate)
    baseline_timing = summarize_timings(baseline.get("measured_step_ms", ()))
    candidate_timing = summarize_timings(candidate.get("measured_step_ms", ()))
    return {
